...
const CrowCoinCrowdsale = artifacts.require('CrowCoinCrowdsale');
const CrowCoin = artifacts.require('CrowCoin');

contract('CrowCoinCrowdsale', function ([owner, wallet, investor]) {
  ...
  beforeEach(async function () {
    this.token = await CrowCoin.new();
    this.crowdsale = await CrowCoinCrowdsale.new(
        rate, cap, this.token.address
    );
  });

  it('should deploy', async function(){
    const ico = await CrowCoinCrowdsale.new(rate, cap, this.token);
  })
